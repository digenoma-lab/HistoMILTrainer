...
from histomil.train_transfer import train_transfer, test

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


class GridSearchTransfer:
    """Grid search for MIL models with cross-validation."""
    ...
    def __init__(self, folds, features_path, splits_dir, csv_path, results_dir, feature_extractor="uni_v2",
                 ...
                 *, model_params_path, transfer_mode, pretrained_checkpoint=None, partial_unfreeze_modules=2,):
        """
        ...
        Args:
            ...
            model_params_path: JSON with fixed pretrained model parameters
            transfer_mode: Transfer strategy: scratch, head_only, partial or full
            pretrained_checkpoint: Source checkpoint required for head_only, partial and full
        """
        self.folds = folds
        ...
        self.model_params_path = os.path.realpath(model_params_path)
        ...
        self.pretrained_checkpoint = (os.path.realpath(pretrained_checkpoint) if pretrained_checkpoint is not None else None)
        self.partial_unfreeze_modules = partial_unfreeze_modules
        self.logger = logging.getLogger(__name__)
        self.logger.info("Initializing transfer training")
        ...
        self.logger.info(f"  - Feature extractor: {self.feature_extractor}")
        self.logger.info(f"  - MIL model: {self.mil}")
        self.logger.info(f"  - Transfer mode: {self.transfer_mode}")
        self.logger.info(f"  - Model parameters: {self.model_params_path}")
        self.logger.info(
        ...
        if self.transfer_mode == "scratch":
        ...
        elif self.transfer_mode == "head_only":
        ...
        elif self.transfer_mode == "partial":
            if self.pretrained_checkpoint is None:
            ...
            raise ValueError(
                ...
                f"Se recibió: {self.transfer_mode}"
            )

        if not os.path.isfile(self.model_params_path):
            raise FileNotFoundError(
                f"No existe model_params_path: {self.model_params_path}"
            )
        ...
        if not isinstance(model_params, dict) or not model_params:
            raise ValueError(
                "model_params_path debe contener un objeto JSON no vacío."
            )

        self.model_params = model_params

        # Se conserva como lista para reutilizar el flujo original,
        # pero contiene una única configuración fija.
        self.param_combinations = [self.model_params]

        self.logger.info(
            f"Fixed model parameters: {self.model_params}"
        )
    ...
    def run(self):
        ...
        for params in self.param_combinations:
            ...
            for fold in range(self.folds):
                ...
                _, train_metrics, checkpoint = train_transfer(
                    model=mil,
